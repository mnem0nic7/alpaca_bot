...
PAPER_APPROVED_STRATEGIES_RESOLVED="${PAPER_APPROVED_STRATEGIES:-$PROFIT_PROBE_STRATEGY}"
if resolved_approved_strategies="$(bash ./scripts/resolve_paper_approved_strategies.sh "$ENV_FILE" "$PROFIT_PROBE_STRATEGY" 2>/dev/null)" \
  && [[ -n "$resolved_approved_strategies" ]]; then
  PAPER_APPROVED_STRATEGIES_RESOLVED="$resolved_approved_strategies"
fi
PROFIT_PROBE_STRATEGIES="${PROFIT_PROBE_STRATEGIES:-$PAPER_APPROVED_STRATEGIES_RESOLVED}"
