...
PAPER_APPROVED_STRATEGIES_RESOLVED="${PAPER_APPROVED_STRATEGIES:-$DEPLOY_DECISION_DRY_RUN_STRATEGY}"
if resolved_approved_strategies="$(bash "$ROOT_DIR/scripts/resolve_paper_approved_strategies.sh" "$ENV_FILE" "$DEPLOY_DECISION_DRY_RUN_STRATEGY" 2>/dev/null)" \
  && [[ -n "$resolved_approved_strategies" ]]; then
  PAPER_APPROVED_STRATEGIES_RESOLVED="$resolved_approved_strategies"
fi
DEPLOY_DECISION_DRY_RUN_STRATEGIES="${DEPLOY_DECISION_DRY_RUN_STRATEGIES:-${PAPER_READINESS_DECISION_DRY_RUN_STRATEGIES:-$PAPER_APPROVED_STRATEGIES_RESOLVED}}"
...
DEPLOY_EXPECT_ENABLED_STRATEGIES="${DEPLOY_EXPECT_ENABLED_STRATEGIES:-$PAPER_APPROVED_STRATEGIES_RESOLVED}"
