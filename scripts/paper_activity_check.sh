...
PAPER_APPROVED_STRATEGIES_RESOLVED="${PAPER_APPROVED_STRATEGIES:-$PAPER_ACTIVITY_STRATEGY}"
if resolved_approved_strategies="$(bash ./scripts/resolve_paper_approved_strategies.sh "$ENV_FILE" "$PAPER_ACTIVITY_STRATEGY" 2>/dev/null)" \
  && [[ -n "$resolved_approved_strategies" ]]; then
  PAPER_APPROVED_STRATEGIES_RESOLVED="$resolved_approved_strategies"
fi
PAPER_ACTIVITY_STRATEGIES="${PAPER_ACTIVITY_STRATEGIES:-$PAPER_APPROVED_STRATEGIES_RESOLVED}"
