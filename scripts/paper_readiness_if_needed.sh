...
PAPER_APPROVED_STRATEGIES_RESOLVED="${PAPER_APPROVED_STRATEGIES:-$PAPER_READINESS_DECISION_DRY_RUN_STRATEGY}"
if resolved_approved_strategies="$(bash ./scripts/resolve_paper_approved_strategies.sh "$ENV_FILE" "$PAPER_READINESS_DECISION_DRY_RUN_STRATEGY" 2>/dev/null)" \
  && [[ -n "$resolved_approved_strategies" ]]; then
  PAPER_APPROVED_STRATEGIES_RESOLVED="$resolved_approved_strategies"
fi
PAPER_READINESS_DECISION_DRY_RUN_STRATEGIES="${PAPER_READINESS_DECISION_DRY_RUN_STRATEGIES:-$PAPER_APPROVED_STRATEGIES_RESOLVED}"
