...
def test_proof_status_lock_skip_uses_recent_proof_status_audit(tmp_path: Path) -> None:
    ...
    assert (
        "approval_marker_command_confirm_env=PROMOTE_VALIDATED_STRATEGY_CONFIRM"
        in result.stdout
    )
    assert (
        "approval_marker_command_dry_run_env=PROMOTE_VALIDATED_STRATEGY_DRY_RUN"
        in result.stdout
    )
...
def test_paper_proof_status_labels_pre_start_window_with_completed_session() -> None:
    ...
    assert (
        ...
        "PROMOTE_VALIDATED_STRATEGY_DRY_RUN"
    ) in script
