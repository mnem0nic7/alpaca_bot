...
def test_cron_runs_session_guard_profit_probe_then_nightly() -> None:
    ...
    assert "approve_validated_strategy_marker.sh" in cron_health
...
def test_second_strategy_basket_scan_is_read_only_prefilter_tool() -> None:
    ...
    assert "SECOND_STRATEGY_OPTION_REPLAY_MIN_POINTS_PER_SESSION" in script
    assert "OPTION_SNAPSHOT_MIN_POINTS_PER_SESSION" in script
...
def test_locked_check_wrapper_audits_lock_skips() -> None:
    ...
    assert "payload->>'exit_code' IN ('0', '43')" in lock_skip
    ...
    assert "proof_earliest_active_days_met_session" in lock_skip
    ...
    assert "proof_execution_quality_status" in lock_skip
    assert "execution_quality_line" in lock_skip
    assert "latest_execution_quality_line" in lock_skip
    assert "paper proof execution quality:" in lock_skip
    assert "proof_execution_quality_entry_dispatch_delay" in lock_skip
    assert "proof_execution_quality_current_posture_entry_dispatch_delay" in lock_skip
    assert "paper proof nightly automation:" in lock_skip
    assert "nightly_automation_line" in lock_skip
    assert "latest_nightly_automation_line" in lock_skip
    assert "proof_nightly_status" in lock_skip
    assert "proof_nightly_run_age_limit_status" in lock_skip
    assert "proof_nightly_log_stall_status" in lock_skip
    assert "proof_second_strategy_scan_status" in lock_skip
    ...
    assert '"$latest_status" == "pending" \\' in lock_skip
    assert '"$latest_exit_code" == "0" || "$latest_exit_code" == "43"' in lock_skip
    assert '&& "$latest_proof" == "pending"' in lock_skip
...
def test_proof_status_lock_skip_uses_recent_proof_status_audit(tmp_path: Path) -> None:
    ...
    assert (
        ...
        "overall_reason=awaiting_overall_blockers blockers=none "
    ...
    assert "log_age_minutes=411" in result.stdout
    assert "max_age_minutes=390" in result.stdout
    assert "run_age_limit_status=not_applicable_idle" in result.stdout
    assert "log_stall_status=not_applicable_idle" in result.stdout
    assert (
        "stage=apply_candidate_2026-07-09T00:16:04Z_"
        "PAPER_PROOF_FREEZE=true__skipping_candidate_auto-apply."
    ) in result.stdout
    assert "second_strategy_scan_status=ok" in result.stdout
    ...
    assert "paper proof active day detail: status=blocked" in result.stdout
    assert "sessions=2026-07-07,2026-07-08" in result.stdout
    assert "trades_by_session=2026-07-07:2,2026-07-08:1" in result.stdout
    assert "remaining_trades_per_required_active_day=9.0" in result.stdout
    assert "earliest_active_days_met_session=2026-07-13" in result.stdout
    assert "paper proof strategy diversification: status=blocked" in result.stdout
    assert "approval_marker_action_status=ready" in result.stdout
    assert "validated_unapproved_stock_candidates=ema_pullback" in result.stdout
    assert (
        "paper proof second strategy promotion action: "
        "status=ready_needs_write_access"
    ) in result.stdout
    assert (
        "confirmation=approve-ema_pullback-paper-promotion-sha256-"
        "df05497c8f595488d5da5f6ef05205c0b3122b73df4cb3fd0b1ed3f98a71aaf9"
    ) in result.stdout
    assert "approval_marker_command_status=ready" in result.stdout
    assert (
        "approval_marker_command_script=./scripts/approve_validated_strategy_marker.sh"
        in result.stdout
    )
    assert (
        "approval_marker_command_confirm_env=APPROVE_VALIDATED_STRATEGY_MARKER_CONFIRM"
        in result.stdout
    )
    assert (
        "approval_marker_command_dry_run_env=APPROVE_VALIDATED_STRATEGY_MARKER_DRY_RUN"
    ...
    assert "approval_marker_command_dry_run_value=false" in result.stdout
    assert (
        "approval_marker_command_approval_only_env="
        "PROMOTE_VALIDATED_STRATEGY_APPROVAL_ONLY"
    ) in result.stdout
    assert "approval_marker_command_approval_only_value=true" in result.stdout
    assert (
        "approval_marker_command_evidence_root="
        "/var/lib/alpaca-bot/nightly/second_strategy"
    ) in result.stdout
    assert "approval_marker_command_deploy_script=./scripts/deploy.sh" in result.stdout
    assert (
        "paper proof second strategy approval quick command: status=ready "
        "command=APPROVE_VALIDATED_STRATEGY_MARKER_CONFIRM="
    ...
    assert "broker_flat_status=ok" in result.stdout
    assert "promotion_handoff_status=ready_needs_privileged_env_write" in result.stdout
    assert "promotion_handoff_step=env_allowlist_update" in result.stdout
    assert "promotion_env_keys=PAPER_APPROVED_STRATEGIES," in result.stdout
    assert "approval_marker_writable=true" in result.stdout
    assert (
    ...
    assert (
        "paper proof execution quality: status=ok "
        "warnings=historical_entry_fill_rate"
    ) in result.stdout
    assert "entry_fill_rate_status=historical_below_minimum_current_ok" in result.stdout
    assert "effective_entry_fill_rate=0.33" in result.stdout
    assert "entry_dispatch_delay=count:14,late:1,max_s:710.5" in result.stdout
    assert (
...
def test_proof_status_lock_skip_accepts_recent_skipped_proof_status_audit(
    ...
    docker.write_text(
        "#!/usr/bin/env bash\n"
        "cat >/dev/null\n"
        "echo 'paper_proof_status_latest=skipped|0|pending|ready|none|sample_trades|sample_trades,eod_loss_share|sample_trades,profit_factor|sample_trades|sample_trades,eod_loss_share|awaiting_completed_proof_session|awaiting_overall_blockers|none|pending|0|10|0.00|0.01|none|none|ok|980|2026-06-26|none|0|0|none|2026-06-28T06:37:20.499132Z|0'\n"
    )
    docker.chmod(0o755)
...
def test_proof_status_lock_skip_preserves_invocation_overrides(tmp_path: Path) -> None:
    ...
    docker.write_text(
        "#!/usr/bin/env bash\n"
        "cat >/dev/null\n"
        "echo 'paper_proof_status_latest=pending|43|pending|ready|none|sample_trades|sample_trades,eod_loss_share|sample_trades,profit_factor|sample_trades|sample_trades,eod_loss_share|awaiting_completed_proof_session|awaiting_overall_blockers|none|pending|0|12|0.00|2.34|none|none|ok|980|2026-06-26|none|0|0|none|2026-06-28T06:37:20.499132Z|0'\n"
    )
    docker.chmod(0o755)
...
def test_proof_status_lock_skip_fails_without_recent_evidence(tmp_path: Path) -> None:
    ...
    docker.write_text(
        "#!/usr/bin/env bash\n"
        "cat >/dev/null\n"
        "echo 'paper_proof_status_latest=pending|43|pending|ready|none|sample_trades|sample_trades,eod_loss_share|awaiting_completed_proof_session|awaiting_overall_blockers|none|pending|0|10|0.00|0.01|none|none|ok|980|2026-06-26|none|0|0|none|2026-06-28T06:37:20.499132Z|31'\n"
    )
    docker.chmod(0o755)
...
def test_run_check_with_audit_records_scheduled_check_result() -> None:
    ...
    assert (
        'AUDIT_PROOF_NIGHTLY_AUTOMATION_LINE="'
        '$proof_nightly_automation_line"'
    ) in script
    ...
    assert (
        'AUDIT_PROOF_STRATEGY_DIVERSIFICATION_LINE="'
        '$proof_strategy_diversification_line"'
    ) in script
    assert (
    ...
    assert 'AUDIT_PROOF_EXECUTION_QUALITY_LINE="$proof_execution_quality_line"' in script
    ...
    assert "-e AUDIT_PROOF_NIGHTLY_AUTOMATION_LINE" in script
    ...
    assert 'proof_nightly_automation_line="$(grep -E' in script
    ...
    assert "PROOF_NIGHTLY_AUTOMATION_FIELDS" in script
    assert '"status": "proof_nightly_status"' in script
    assert '"log_age_minutes": "proof_nightly_log_age_minutes"' in script
    assert '"max_age_minutes": "proof_nightly_max_age_minutes"' in script
    assert '"run_age_limit_status": "proof_nightly_run_age_limit_status"' in script
    assert '"log_stall_status": "proof_nightly_log_stall_status"' in script
    assert '"stage": "proof_nightly_stage"' in script
    assert (
    ...
    assert "PROOF_EXECUTION_QUALITY_FIELDS" in script
    ...
    assert '"status": "proof_execution_quality_status"' in script
    assert '"warnings": "proof_execution_quality_warnings"' in script
    assert (
        '"entry_fill_rate_status": '
        '"proof_execution_quality_entry_fill_rate_status"'
    ) in script
    assert (
    ...
    assert "proof_execution_quality_current_posture_entry_dispatch_delay" in script
    ...
    assert 'os.environ.get("AUDIT_PROOF_NIGHTLY_AUTOMATION_LINE", "")' in script
...
def test_paper_proof_status_is_read_only(tmp_path: Path) -> None:
    ...
    fake_docker.write_text(
        ...
        "  printf 'paper proof nightly automation: status=idle lock_status=missing pid=none source=none age_minutes=none log_age_minutes=none max_age_minutes=390 stall_minutes=90 run_age_limit_status=not_applicable_idle log_stall_status=not_applicable_idle stage=none detail=none\\n'\n"
...
def test_paper_proof_status_labels_pre_start_window_with_completed_session() -> None:
    ...
    assert "overall_reason={proof_overall_reason}" in script
    ...
    assert "remaining_trades_per_required_active_day=" in script
    assert "{remaining_trades_per_required_active_day_text}" in script
    assert "sessions={active_trade_sessions_text}" in script
    assert "trades_by_session={trade_count_by_session_text}" in script
    assert "future_sessions={active_day_future_sessions_text}" in script
    assert "earliest_active_days_met_session=" in script
    assert "{earliest_active_days_met_session_text}" in script
    assert "projection_status={active_day_projection_status}" in script
    ...
    assert "PROOF_STATUS_OPTION_REPLAY_MIN_POINTS_PER_SESSION" in script
    assert (
        "PROOF_STATUS_OPTION_REPLAY_MIN_POINTS_PER_SESSION must be a positive integer"
        in script
    )
    ...
    assert "run_age_limit_status={nightly_run_age_limit_status}" in script
    assert "log_stall_status={nightly_log_stall_status}" in script
    ...
    assert "active_trade_session_dates = sorted(set(exit_sessions))" in script
    assert "active_trade_day_count = len(active_trade_session_dates)" in script
    ...
    assert "def option_snapshot_session_point_counts" in script
    ...
    assert "session_point_counts[session] >= min_points_per_session" in script
    assert (
    ...
    assert "simulate_buy_stop_limit_fill" in script
    ...
    assert "current_posture_only" in script
    assert "include_late=not current_posture_only" in script
    assert "d.signal_bar_close / NULLIF(d.entry_level, 0) - 1" in script
    ...
    assert '"insufficient_sessions"' in script
    assert 'else "insufficient_coverage"' in script
    assert "option_snapshot_replay_session_count >= option_replay_min_sessions" in script
    assert "snapshot_sessions={option_snapshot_summary['snapshot_session_count']}" in script
    ...
    assert "min_points_per_session={option_snapshot_summary['min_points_per_session']}" in script
    assert "session_points={safe_status_value(option_snapshot_summary['session_points'])}" in script
    assert "undercovered_sessions={safe_status_value(option_snapshot_summary['undercovered_sessions'])}" in script
    ...
    assert "approval_marker_action_status={approval_marker_action_status}" in script
    assert "approval_marker_command_status={approval_marker_command_status}" in script
    assert (
        "approval_marker_command_script=./scripts/approve_validated_strategy_marker.sh"
        in script
    )
    ...
    assert "approval_marker_command_dry_run_value=false" in script
    assert (
        "approval_marker_command_approval_only_env="
        "PROMOTE_VALIDATED_STRATEGY_APPROVAL_ONLY"
    ) in script
    assert "approval_marker_command_approval_only_value=true" in script
    assert (
        "approval_marker_command_evidence_root="
    ...
    assert "promotion_handoff_status={promotion_handoff_status}" in script
    assert "promotion_handoff_step={promotion_handoff_step}" in script
    assert "promotion_env_keys={promotion_env_keys_csv}" in script
