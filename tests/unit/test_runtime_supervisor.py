...
def test_runtime_supervisor_close_stops_background_trade_update_stream() -> None:
    ...
    assert stream.run_started.wait(timeout=1.0)
    stream_thread = supervisor._stream_thread
...
def test_runtime_supervisor_blocks_readiness_with_wrong_decision_dry_run_strategy(
    monkeypatch,
) -> None:
    module, RuntimeSupervisor, _SupervisorCycleReport = load_supervisor_api()
    settings = make_settings(
    ...
    readiness_event = AuditEvent(
        event_type="scheduled_check_completed",
        payload=paper_readiness_pass_payload(
            decision_dry_run_strategy="other_flag",
        ),
        created_at=datetime(2026, 6, 29, 13, 55, tzinfo=timezone.utc),
    )
    supervisor_event = AuditEvent(
...
def test_runtime_supervisor_blocks_list_readiness_for_wrong_proof_start(
    monkeypatch,
) -> None:
    module, RuntimeSupervisor, _SupervisorCycleReport = load_supervisor_api()
    settings = make_settings({"PAPER_PROOF_FREEZE": "true"})
    ...
    audit_store = RecordingAuditEventStore(
        events=[
            ...
            AuditEvent(
                event_type="supervisor_started",
                payload={},
                created_at=datetime(2026, 6, 29, 14, 5, tzinfo=timezone.utc),
            ),
        ]
    )
    runtime = make_runtime_context(
    ...
    audit_store = RecordingAuditEventStore(
        events=[
            AuditEvent(
                ...
                created_at=datetime(2026, 6, 29, 13, 20, tzinfo=timezone.utc),
            ),
            AuditEvent(
                event_type="scheduled_check_completed",
                payload={
...
def test_quiet_trade_stream_is_not_restarted_without_order_events(monkeypatch) -> None:
    """A live event-driven stream can be quiet indefinitely between order updates."""
    ...
    supervisor._last_stream_event_at = base_now - timedelta(hours=2)
    ...
    try:
        ...
        assert stale_events == []
        assert stream.stop_calls == 1  # orderly run_forever shutdown only
...
def test_stream_restart_replaces_stream_when_stop_does_not_join(monkeypatch) -> None:
    ...
    try:
        ...
        supervisor._restart_stream_thread(timestamp=now, reason="thread_unresponsive")
