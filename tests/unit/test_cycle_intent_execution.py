...
class RecordingBrokerWithPositions(RecordingBroker):
    def __init__(
        self,
        *,
...
def test_execute_cycle_intents_skips_update_stop_when_broker_position_missing() -> None:
    execute_cycle_intents = load_cycle_intent_execution_api()
    settings = make_settings()
    now = datetime(2026, 4, 24, 19, 30, tzinfo=timezone.utc)
    active_stop = OrderRecord(
    ...
    broker = RecordingBrokerWithPositions(positions=[])
    cycle_result = CycleResult(
        as_of=now,
        intents=[
            CycleIntent(
    ...
    assert broker.list_position_calls == 1
    assert broker.replace_calls == []
...
def test_execute_cycle_intents_updates_stop_when_broker_position_snapshot_fails() -> None:
    execute_cycle_intents = load_cycle_intent_execution_api()
    settings = make_settings()
    now = datetime(2026, 4, 24, 19, 30, tzinfo=timezone.utc)
    active_stop = OrderRecord(
    ...
    broker = RecordingBrokerWithPositions(
        positions=[],
        list_positions_raises=RuntimeError("position snapshot unavailable"),
    )
    cycle_result = CycleResult(
        as_of=now,
        intents=[
    ...
    assert runtime.order_store.saved[0].stop_price == 111.7
    assert runtime.position_store.saved[0].stop_price == 111.7
    assert report.replaced_stop_count == 1
    assert report.submitted_stop_count == 0
