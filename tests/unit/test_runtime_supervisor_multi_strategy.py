...
def test_cross_strategy_symbol_dedup_blocks_second_strategy_entry():
    """Strategy B must not enter a symbol already held by Strategy A."""
    ...
    # Two strategies active: breakout + momentum
    flags = [
        StrategyFlag(
            strategy_name="breakout",
            trading_mode=TradingMode.PAPER,
    ...
    # AAPL is held by "breakout"; "momentum" should not be able to enter it.
    # position_store.list_all() returns raw DB record SimpleNamespaces.
    runtime.position_store = SimpleNamespace(
        list_all=lambda **_: [
            SimpleNamespace(
                symbol="AAPL",
                quantity=10,
                ...
                highest_price=0.0,
                strategy_name="breakout",
                opened_at=datetime(2026, 1, 2, 10, 0, tzinfo=timezone.utc),
            )
        ],
    ...
    # At least one strategy cycle call should be made.
    assert len(received_working_symbols) >= 1
    # Every strategy after the first must see AAPL in its working_order_symbols
    # so the engine's dedup guard prevents a cross-strategy duplicate entry.
    for working_symbols in received_working_symbols[1:]:
